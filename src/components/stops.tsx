...
const directionButtonStyle = (isActive: boolean): React.CSSProperties => ({
  flex: 1,
  padding: "8px 16px",
...
const stopListStyle = { display: "flex", flexDirection: "column" as const, gap: "16px" };
const stopInfoRowStyle = { display: "flex", alignItems: "center" };
...
const StopsList: React.FC<StopsListProps> = ({
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={directionTabStyle}>
        <button onClick={() => onDirectionChange(0)} style={directionButtonStyle(activeDirection === 0)}>
          Direction 1
        </button>
        <button onClick={() => onDirectionChange(1)} style={directionButtonStyle(activeDirection === 1)}>
          Direction 2
        </button>
      </div>
      ...
      {currentStops.length === 0 ? (
        ...
        <div style={stopListStyle}>
          {currentStops.map((stop, index) => (
            <div key={stop.stop_id} style={stopItemStyle} onClick={() => onStopSelect(stop)}>
              <div style={stopInfoRowStyle}>
