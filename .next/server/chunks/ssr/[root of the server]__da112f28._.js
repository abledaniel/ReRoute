...
const Page = ()=>{
    ...
    const fetchRoutes = async (lat, lng, setRoutes)=>{
        try {
            ...
            const { data: nearbyStops, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('stops').select('stop_id, stop_name, stop_lat, stop_lon').filter('stop_lat', 'gte', formattedLat - 0.01).filter('stop_lat', 'lte', formattedLat + 0.01).filter('stop_lon', 'gte', formattedLng - 0.01).filter('stop_lon', 'lte', formattedLng + 0.01);
            if (error) {
            ...
            const { data: stopTimes, error: stopTimesError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('stop_times').select('trip_id, stop_id').in('stop_id', stopIds).limit(100);
            if (stopTimesError) {
            ...
            const { data: trips, error: tripsError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('trips').select('route_id, trip_id').in('trip_id', tripIds).limit(100);
            if (tripsError) {
            ...
            // Get route details
            const { data: routes, error: routesError } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from('routes').select('route_id, route_short_name, route_long_name').in('route_id', routeIds).limit(20);
            if (routesError) {
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__da112f28._.js.map